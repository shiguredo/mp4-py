...
`Mp4SampleEntry*` 系コンストラクタがフィールドの値域を検証しないため、コアクレートのビット幅を超える値がエラーにならずに「黙って切り捨て」または「隣接フィールドの値の汚染」を起こし、不正な MP4 が静かに生成される状態を解消する。既存の検証 (Tx3g の 4 バイト固定長、timescale=0) と比べても不整合だった。
