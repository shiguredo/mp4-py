...
`Mp4MuxSample(data=...)` などに `int` を渡した場合に、エラーにならず「その値の長さのゼロ埋めバイト列」として静かに書き込まれる問題を解消する。型ミスによるデータ破壊をエラーとして検出できるようにする。`data=2**30` のような巨大な int では 1GB 規模のゼロ埋めバッファが確保されるため、メモリ消費面でも危険だった。
