...
公開 API のエラーパス・入力経路のテストが欠落している状態を解消し、リグレッションを検出できるようにする。コード本体のバグ修正は各 issue で行い、本 issue はテスト整備のみに絞る。

## 優先度根拠

Medium。

- エラーパス (破損データ検出のガード・コンストラクタの検証・入力変換) がテストされていないため、リグレッションを検出できない
- 修正コストは中 (テスト追加のみ)
