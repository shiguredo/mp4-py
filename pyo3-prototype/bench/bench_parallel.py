"""Free-Threading 環境で並列 mux をやったときのスループット比較。

複数スレッドがそれぞれ独立した Muxer を持って並列に mux する場合、
GIL が無い分だけ真の並列実行になっているかを比率で確認する。

- baseline: 1 スレッドで N 回シリアル実行
- parallel: T スレッドで各 N/T 回実行
- 理想値: T スレッドなら 1/T の時間で終わる (実際は Mutex やアロケータで下振れ)
"""

import io
import statistics
import sys
import time
from concurrent.futures import ThreadPoolExecutor

import mp4 as nb
import mp4_pyo3 as p3


NUM_SAMPLES = 500
SAMPLE_SIZE = 4096
TIMESCALE = 1_000_000
SAMPLE_DURATION = 33_333
TOTAL_JOBS = 32  # 総 mux 回数
THREADS = 8


def make_samples() -> list[bytes]:
    return [bytes((i * 17 + j) & 0xFF for j in range(SAMPLE_SIZE)) for i in range(NUM_SAMPLES)]


def one_mux_nanobind(samples: list[bytes]) -> None:
    output = io.BytesIO()
    muxer = nb.Mp4FileMuxer(output)
    entry = nb.Mp4SampleEntryVp08(width=1920, height=1080, bit_depth=8, chroma_subsampling=1)
    for data in samples:
        muxer.append_sample(
            nb.Mp4MuxSample(
                track_kind="video",
                sample_entry=entry,
                keyframe=True,
                timescale=TIMESCALE,
                duration=SAMPLE_DURATION,
                data=data,
            )
        )
    muxer.finalize()


def one_mux_pyo3(samples: list[bytes]) -> None:
    output = io.BytesIO()
    muxer = p3.Mp4FileMuxer(output)
    entry = p3.Mp4SampleEntryVp08(width=1920, height=1080, bit_depth=8, chroma_subsampling=1)
    for data in samples:
        muxer.append_sample(
            p3.Mp4MuxSample(
                track_kind="video",
                sample_entry=entry,
                keyframe=True,
                timescale=TIMESCALE,
                duration=SAMPLE_DURATION,
                data=data,
            )
        )
    muxer.finalize()


def bench_serial(fn, samples: list[bytes], jobs: int) -> float:
    t0 = time.perf_counter()
    for _ in range(jobs):
        fn(samples)
    return time.perf_counter() - t0


def bench_parallel(fn, samples: list[bytes], jobs: int, threads: int) -> float:
    t0 = time.perf_counter()
    with ThreadPoolExecutor(max_workers=threads) as executor:
        futures = [executor.submit(fn, samples) for _ in range(jobs)]
        for f in futures:
            f.result()
    return time.perf_counter() - t0


def main() -> None:
    samples = make_samples()
    print(f"Python: {sys.version.split()[0]}, GIL enabled: {sys._is_gil_enabled()}")
    print(
        f"jobs={TOTAL_JOBS}, samples/job={NUM_SAMPLES}, sample_size={SAMPLE_SIZE} B, "
        f"threads={THREADS}"
    )
    print()

    # ---- warmup ----
    for _ in range(2):
        one_mux_nanobind(samples)
        one_mux_pyo3(samples)

    def measure(name: str, fn) -> None:
        serial_runs = [bench_serial(fn, samples, TOTAL_JOBS) for _ in range(3)]
        parallel_runs = [bench_parallel(fn, samples, TOTAL_JOBS, THREADS) for _ in range(3)]
        s = statistics.median(serial_runs)
        p = statistics.median(parallel_runs)
        speedup = s / p if p > 0 else float("inf")
        print(f"{name}")
        print(f"  serial ({TOTAL_JOBS} jobs, 1 thread):    {s * 1000:8.1f} ms")
        print(
            f"  parallel ({TOTAL_JOBS} jobs, {THREADS} threads): {p * 1000:8.1f} ms "
            f"(speedup {speedup:.2f}x / ideal {THREADS}x)"
        )
        print(f"  parallel efficiency: {speedup / THREADS * 100:.0f}%")
        print()

    measure("nanobind", one_mux_nanobind)
    measure("PyO3", one_mux_pyo3)


if __name__ == "__main__":
    main()
